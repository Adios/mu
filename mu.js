...
$(document).on('ready', function () {
	...
	mu.player.on($.jPlayer.event.play, function () {
		$('#muground').attr('src', 'muon.jpg');
	})
	.on($.jPlayer.event.pause, function () {
		$('#muground').attr('src', 'muoff.jpg');
	});
...
var muuun = (function ($) {
	...
	function bind () {
		board.on('resize.muuun', function () {
			var	w = $(window).width() + 'px';
				lhs = solve($(window).height(), row);

			board.find('li > div').each(function () {
				var lh = lhs.pop();
				$(this).css({
					'width': w,
					'height': lh,
		...
		.on('boardCreated.muuun', function () {
			board.empty().append(f);
			m = board.find('marquee').marquee();
			board.trigger('resize.muuun');

			for (var name in meta)
				meta[name] = meta[name].find('div div');
		})
		.on('click', function () {
			if (marqueeing) {
				marqueeing = false;
				m.toggleClass('shock').trigger('stop');
			} else {
				marqueeing = true;
				m.toggleClass('shock').trigger('start');
			}
		});
		...
		$(window).on('resize orientationChanged', function () {
			...
			triggerReszie = setTimeout(function () { board.trigger('resize.muuun'); }, 500);
	...
	function solve (sum, n) {
		...
		x = sum * (0.618) / (Math.pow(1.618, n) - 1);

		for (var i = 0; i < n; i++, x *= 1.618)
			ary.push(Math.floor(x) + 'px');
	...
	function create (data) {
		...
		for (var name in data) {
			...
			$(m).text(data[name]).attr('direction', (Math.random() < 0.5) ? 'left' : 'right');
		}
		board.trigger('boardCreated.muuun');
	}
})(jQuery);
...
// https://gist.github.com/2484402 (2012.08.07) jquery.marquee.js
